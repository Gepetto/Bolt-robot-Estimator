...
def main():
    ...
    ToPlot = "position, attitude, contact"
    ...
    # load traj generated by simulation  NEW SIMU OUTPUT
    device.LoadSimulatedDataDirect(Reader=reader, style="standing")
    # init estimator
    logging = True
    ...
    True_w = reader.Get("omega")
    True_quat = reader.Get("theta_euler")
    True_x = reader.Get("x")
    ...
    if "attitude" in ToPlot :
        ...
        grapher.SetLegend(["true quat", "quat out"], ndim=4)
        grapher.CompareNDdatas([True_quat.T, quat_out], datatype='quaternion', title='Estimator quaternions', mitigate=[1])
